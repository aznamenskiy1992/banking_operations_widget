...
def test_log_success_operation_in_file_for_get_mask_card_number_with_log_decorator(
    ...
    result = get_mask_card_number(example_input_card_for_get_mask_card_number)
...
def test_log_error_operation_in_file_for_get_mask_card_number_with_log_decorator(
    ...
    try:
        ...
        with open(example_path_to_log_file, "r", encoding="utf-8") as file:
            ...
            assert (
                content[-1].strip()
                == f"get_mask_card_number error: Не указан номер карты или счёта. Inputs: {example_input_none_for_get_mask_card_number}"
            )
...
def test_log_error_operation_in_file_for_filter_by_state_with_log_decorator(
    ...
    try:
        ...
        with open(example_path_to_log_file, "r", encoding="utf-8") as file:
            ...
            assert (
                content[-1].strip()
                == f"filter_by_state error: \"В словарях нет ключа 'state'\". Inputs: {example_input_none_for_filter_by_state}"
            )
...
def test_log_error_operation_in_console_for_filter_by_state_with_log_decorator(capsys):
    ...
    try:
        ...
        assert (
            captured.out
            == "filter_by_state error: \"В словарях нет ключа 'state'\". Inputs: [{'id': 41428829, 'date': '2019-07-03T18:35:29.512364'}, {'id': 939719570, 'date': '2018-06-30T02:08:58.425572'}, {'id': 594226727, 'date': '2018-09-12T21:27:25.241689'}, {'id': 615064591, 'date': '2018-10-14T08:21:33.419441'}]\n"
        )
